...
async function seed() {
	...
	if (
		...
		console.error(
			"❌ Missing Contentful environment variables (NEXT_PUBLIC_CONTENTFUL_PAT, NEXT_PUBLIC_CONTENTFUL_SPACE_ID)",
		);
		process.exit(1);
	}
	...
	try {
		...
		console.log(
			`Connected to Space: ${spaceId}, Environment: ${environmentId}`,
		);

		const problemsData = JSON.parse(
			fs.readFileSync(PROBLEMS_FILE_PATH, "utf-8"),
		);
