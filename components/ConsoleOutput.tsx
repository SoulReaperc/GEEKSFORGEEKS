...
const ConsoleOutput = ({
	...
	if (submissionResult) {
		...
		return (
			<div className="h-full w-full flex flex-col bg-gradient-to-b from-black to-black/95 text-white overflow-hidden">
				{/* Header */}
				<div
					className={`p-4 border-b ${isSuccess ? "border-green-500/30 bg-green-500/10" : "border-red-500/30 bg-red-500/10"}`}
				>
				...
				<div className="flex-1 overflow-auto p-6 space-y-4">
					<p
						className={`text-sm ${isSuccess ? "text-gray-300" : "text-red-300"}`}
					>
					...
					{isSuccess && (
						<div className="bg-green-500/10 border border-green-500/30 rounded-xl p-4">
							<p className="text-sm text-green-300 mb-2">
								...
								Your solution has been saved and points awarded.
							</p>
							<p className="text-xs text-green-300/80">
								Go back to{" "}
								<Link
									href="/practice"
									className="underline hover:text-green-200 font-semibold"
								>
									Practice Page
								</Link>{" "}
								to see your updated stats and checkmark.
							</p>
						</div>
					)}
					...
					{isSuccess && grading && (
						<div className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-4">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
								{Object.entries(grading.details).map(
									([key, value]: [string, GradingDetail]) => (
										<div
											key={key}
											className="bg-black/50 p-3 rounded-lg border border-white/10"
										>
											<div className="capitalize text-gray-400 text-xs mb-1">
												{key.replace(/_/g, " ")}
											</div>
	...
	return (
		<div className="h-full w-full flex flex-col bg-gradient-to-b from-black to-black/95 text-white overflow-hidden">
			{/* Header */}
			<div
				className={`p-4 border-b ${status === "Passed" ? "border-green-500/30 bg-green-500/10" : "border-red-500/30 bg-red-500/10"}`}
			>
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-3">
						...
						<span
							className={`text-sm font-semibold ${status === "Passed" ? "text-green-400" : "text-red-400"}`}
						>
					...
					<div className="flex items-center gap-2">
						...
						<span
							className={`text-sm font-bold ${status === "Passed" ? "text-green-400" : "text-red-400"}`}
						>
				...
				{status === "Passed" && (
					<div className="mt-3 pt-3 border-t border-green-500/20">
						<p className="text-xs text-green-300/80">
							✅ All test cases passed! Click{" "}
							<span className="font-bold text-green-400">Submit</span> to save
							your solution and earn points.
						</p>
					</div>
				)}
			</div>
							...
							>
								...
								<div className="p-4 space-y-3">
									...
									<div>
										...
										<div
											className={`border rounded-lg p-3 font-mono text-xs ${
												res.passed
