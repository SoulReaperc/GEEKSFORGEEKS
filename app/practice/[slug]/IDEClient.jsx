...
const IDEClient = ({ problem, initialCode }) => {
	...
	const config = difficultyConfig[problem.fields.difficulty] || {
		color: "text-gray-400",
		bg: "bg-gray-500/20",
	...
	return (
		...
		>
			...
			<div className="fixed top-0 left-0 right-0 h-16 bg-black/80 backdrop-blur-xl border-b border-white/10 z-50 flex items-center justify-between px-6">
				<div className="flex items-center gap-4">
					...
					<h1 className="text-lg font-bold text-white">
						{problem.fields.title}
					</h1>
					<span
						className={`inline-flex items-center px-3 py-1 rounded-lg text-xs font-semibold border ${config.border} ${config.bg} ${config.color}`}
					>
			...
			<div className="pt-16 h-screen flex">
				...
				<div
					className={`${isFullscreen ? "hidden" : "w-full md:w-[450px]"} flex flex-col border-r border-white/10 bg-black`}
				>
					{/* Tabs */}
