		...
		<div className="max-w-7xl mx-auto space-y-8">
				...
				<div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
					<div className="flex-1">
						...
						<p className="text-white/50 text-sm md:text-base">
							Create, edit and manage club events & activities
						</p>
					</div>
			...
			{events.length === 0 ? (
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
					{events.map((event) => {
						const title =
							(event.fields.title?.["en-US"] as string | undefined) ||
							"Untitled Event";
						...
						return (
							<div
								key={event.sys.id}
								className="relative group backdrop-blur-xl bg-gradient-to-br from-white/10 to-white/5 border border-white/10 hover:border-purple-500/30 rounded-2xl p-6 transition-all duration-300 hover:shadow-[0_0_30px_rgba(168,85,247,0.2)] overflow-hidden"
							>
								<Link
									href={`/admin/events/${event.sys.id}`}
									className="absolute inset-0 z-0 rounded-2xl"
								/>
								<div className="pointer-events-none relative z-10 w-full h-full flex flex-col">
