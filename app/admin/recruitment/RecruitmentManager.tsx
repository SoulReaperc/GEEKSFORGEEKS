			...
			<div className="flex flex-col md:flex-row gap-4 items-center justify-between">
				<div className="relative w-full md:w-96 group">
					<div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
						<svg
							className="w-5 h-5 text-white/30 group-focus-within:text-white/50 transition-colors"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
			...
			<div className="flex flex-wrap gap-3">
				{availableDomains.map((domain) => (
					<button
						...
						className={`px-6 py-3 rounded-2xl font-semibold transition-all duration-300 border flex items-center gap-2 ${
							activeDomain === domain
								...
								<table className="w-full text-left">
									...
									<tbody className="text-white/80">
										{domainApps.map((row, idx) => (
											...
											>
												<td className="px-4 py-3 font-medium text-white text-sm whitespace-nowrap">
													{row.name}
												</td>
												<td className="px-3 py-3 font-mono text-xs opacity-60 group-hover:opacity-100 transition-opacity whitespace-nowrap">
													{row.reg_no}
												</td>
												<td className="px-3 py-3 text-xs opacity-60 group-hover:opacity-100 transition-opacity truncate max-w-[150px]">
													{row.email_personal}
												</td>
												<td className="px-3 py-3 text-xs opacity-60 group-hover:opacity-100 transition-opacity whitespace-nowrap">
													{row.phone}
												</td>
												...
												<td className="px-3 py-3 text-xs opacity-80 whitespace-nowrap">
													{row.branch}
												</td>
				...
				<div className="flex flex-col md:flex-row items-center justify-between gap-6">
					...
					<div className="flex items-center gap-6">
						<div className="flex items-center gap-3">
							<div
								className={`w-2.5 h-2.5 rounded-full ${isRecruitmentOpen ? "bg-green-500 shadow-[0_0_12px_rgba(34,197,94,0.6)]" : "bg-red-500 shadow-[0_0_12px_rgba(239,68,68,0.6)]"} animate-pulse`}
							/>
							<span
						...
						>
							<span
								className={`inline-block h-7 w-7 transform rounded-full bg-white shadow-xl transition-transform duration-300 ${
									isRecruitmentOpen ? "translate-x-11" : "translate-x-1.5"
								}`}
							/>
						</button>
					</div>
