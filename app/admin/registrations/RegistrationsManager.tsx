...
export default function RegistrationsManager({
	...
	const handleExport = () => {
		...
		link.setAttribute(
			"download",
			`team-registrations-${new Date().toISOString().split("T")[0]}.csv`,
		);
	...
	const handleExportPDF = () => {
		...
		autoTable(doc, {
			startY: 40,
			head: [
				["Team Name", "Event", "College", "Members", "Leader Phone", "Date"],
			],
			body: tableData,
			theme: "grid",
			headStyles: {
		...
		// Save the PDF
		doc.save(
			`team-registrations-${new Date().toISOString().split("T")[0]}.pdf`,
		);
	};
	...
	const handleExportOD = () => {
		if (!selectedEventFilter) {
			alert(
				"Please select a specific event from the filter first to export an OD list.",
			);
			return;
		}

		const filteredByEvent = registrations.filter(
			(r) => r.event_name === selectedEventFilter,
		);
	...
	const stats = {
		...
		totalMembers: filteredRegistrations.reduce(
			(acc, reg) => acc + (reg.member_count || 0),
			0,
		),
		...
		<div className="space-y-6">
						...
						>
							<div className="flex items-start justify-between mb-4">
								<div className="flex-1">
									<h3 className="text-xl font-bold text-white mb-1">
										{registration.team_name || "Untitled Team"}
									</h3>
									<p className="text-orange-400 text-sm font-semibold">
										{registration.event_name || "General Event"}
									</p>
								</div>
							...
							<div className="space-y-3">
								...
								<div className="flex items-center gap-2 text-white/60">
									...
									<span className="text-sm">
										{registration.member_count || 0} Team Members
									</span>
								</div>
								<div className="flex items-center gap-2 text-white/60">
			...
			{selectedRegistration &&
				createPortal(
					...
					>
						<div
							className="backdrop-blur-xl bg-gradient-to-br from-white/10 to-white/5 border border-white/20 rounded-3xl p-8 max-w-3xl w-full max-h-[90vh] overflow-y-auto"
							...
							<div className="flex items-start justify-between mb-6">
								<div>
									...
									<p className="text-orange-400 font-semibold">
										{selectedRegistration.event_name}
									</p>
								</div>
								<button
									onClick={() => setSelectedRegistration(null)}
							...
							<div className="space-y-6">
								<div>
									...
									<p className="text-white text-lg">
										{selectedRegistration.college_name || "N/A"}
									</p>
								</div>
								...
								{selectedRegistration.members &&
									Array.isArray(selectedRegistration.members) && (
										<div>
