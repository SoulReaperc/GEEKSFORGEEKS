...
export const EditEvent: React.FC<EditEventProps> = ({
	...
	const CoverImageManagerAny =
		CoverImageManager as unknown as React.ComponentType<
			Record<string, unknown>
		>;
	const GalleryManagerAny = GalleryManager as unknown as React.ComponentType<
		Record<string, unknown>
	>;
				...
				<div className="anim-in flex justify-between items-center mb-8">
					<div className="flex flex-col gap-1">
						...
						<p className="text-sm text-gray-500">
							Manage details, media and registrations for this event.
						</p>
					</div>
					<button
						onClick={onBack}
								...
								<div className="flex items-center justify-between p-4 rounded-xl bg-black/40 border border-white/10">
									<div className="flex items-center gap-3">
										...
										<div
											...
											onClick={() =>
												setFormData((p) => ({ ...p, isRegOpen: !p.isRegOpen }))
											}
										>
											<div
												className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform ${formData.isRegOpen ? "translate-x-4" : "translate-x-0"}`}
											/>
										</div>
									...
									<span
										className={`text-xs font-bold px-2 py-1 rounded border ${formData.isRegOpen ? "text-emerald-400 border-emerald-500/20 bg-emerald-500/10" : "text-gray-500 border-gray-700 bg-gray-800"}`}
									>
