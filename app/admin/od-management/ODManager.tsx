...
export default function ODManager({ registrations, events }: ODManagerProps) {
	...
	const toggleAll = () => {
		if (
			selectedTeamIds.size === displayedTeams.length &&
	...
	const handleGenerateOD = () => {
		...
		const selectedRegistrations = teamsForEvent.filter((t) =>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
					...
					<div className="relative isolate z-50">
						...
						>
							...
							<ChevronDown
								className={`w-5 h-5 text-white/50 transition-transform ${isDropdownOpen ? "rotate-180" : ""}`}
							/>
						</div>
							...
							{isDropdownOpen && (
								...
								>
									...
									{filteredEvents.map((ev) => {
										const title =
											(ev.fields?.title?.["en-US"] as string | undefined) ||
											"Untitled Event";
