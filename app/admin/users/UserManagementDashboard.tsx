...
export default function UserManagementDashboard({
	...
	const [blacklist, setBlacklist] =
		useState<BlacklistEntry[]>(initialBlacklist);
	...
	const handleBlacklist = async (email: string) => {
		...
		try {
			...
			if (res.error) {
				...
				setBlacklist((prev) => [
					...prev,
					{ email, blocked_at: new Date().toISOString() },
				]);
				router.refresh();
			}
		} catch (error: unknown) {
	...
	const handleUnblacklist = async (email: string) => {
		if (
			!confirm(`Remove ${email} from blacklist? They will regain full access.`)
		)
			return;
											...
											>
												<td className="px-6 py-4">
													<div className="flex items-center gap-3">
														<div
															className={`w-10 h-10 rounded-xl ${isBlacklisted ? "bg-red-500/20 border-red-500/30" : "bg-blue-500/20 border-blue-500/30"} border flex items-center justify-center`}
														>
															<span
																className={`font-bold ${isBlacklisted ? "text-red-400" : "text-blue-400"}`}
															>
