...
interface ContentfulEntry {
	...
	fields: {
		...
		bio?: { "en-US": string };
		photo?: {
			"en-US": { sys: { type: "Link"; linkType: "Asset"; id: string } };
		};
		team?: { "en-US": string };
		year?: { "en-US": number };
		github?: { "en-US": string };
...
const extractRawText = (
	richText: string | ContentfulRichTextNode | null | undefined,
): string => {
	if (typeof richText === "string") return richText;
	if (!richText || !richText.content) return "";
	return richText.content
		.map((block: ContentfulRichTextNode) => {
			if (block.nodeType === "paragraph") {
				return (block.content ?? [])
					.map((text: ContentfulRichTextNode) => text.value || "")
					.join("");
			}
			return "";
		})
...
const getMemberCategory = (
	...
	if (
		r === "member" ||
		r.includes("general member") ||
		r.includes("team member")
	)
		return "Member";

	// ... existing logic ...
	if (r.includes("faculty")) return "Faculty";
	if (r.includes("chairperson") || (r.includes("vice") && r.includes("chair")))
		return "Executive";
...
const ConfirmationModal: React.FC<ConfirmationProps> = ({
	...
	return (
		<div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-6">
			<div
				ref={containerRef}
				className="w-full max-w-sm rounded-2xl border border-white/10 bg-[#0a0a0c] p-6 shadow-2xl ring-1 ring-white/10"
			>
				<div
					className={`mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-${colorClass}-500/20 text-${colorClass}-500 mx-auto`}
				>
					{isProcessing ? (
				...
				<p className="text-center text-sm text-gray-400 mb-6 leading-relaxed">
					{step === 1 ? (
						...
						<span className="text-red-400 font-medium">
							This changes the live database directly. Confirm?
						</span>
					)}
				</p>
				...
				<div className="mt-4 flex justify-center gap-1">
					<div
						className={`h-1.5 w-1.5 rounded-full ${step >= 1 ? `bg-${colorClass}-500` : "bg-gray-700"}`}
					/>
					<div
						className={`h-1.5 w-1.5 rounded-full ${step >= 2 ? `bg-${colorClass}-500` : "bg-gray-700"}`}
					/>
				</div>
			</div>
		</div>
...
const MemberModal: React.FC<MemberModalProps> = ({
	...
	const styles = useMemo(
		() => getTeamStyle(formData.team, formData.category),
		[formData.team, formData.category],
	);
	...
	const handleClose = () => {
		...
		tl.to(contentRef.current, { scale: 0.95, opacity: 0, duration: 0.2 }).to(
			modalRef.current,
	...
	return createPortal(
		<div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
			...
			>
				...
				<div
					className={`absolute top-0 left-0 right-0 h-40 bg-gradient-to-b ${styles.glow} to-transparent opacity-50 pointer-events-none z-0`}
				/>
				...
				{/* Content */}
				<div
					className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar relative z-10"
					data-lenis-prevent
				>
						...
						{isEditing ? (
							<div className="w-full space-y-3 mb-6">
								...
								<div className="grid grid-cols-2 gap-2">
									...
									<select
										value={
											teamOptions.find((t) => formData.team.includes(t)) ||
											"General"
										}
							...
							<>
								...
								<div className="flex flex-wrap items-center justify-center gap-2 mb-6">
									<span
										className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${styles.badge}`}
									>
										{formData.role}
									</span>
								...
								{isEditing ? (
									...
									formData.email && (
										<a
											...
											className="flex items-center gap-3 rounded-xl bg-white/5 p-3 transition-colors hover:bg-white/10 group/item"
										>
								...
								<div className="col-span-2 flex justify-center gap-4 py-2">
									{formData.linkedin && (
										<a
											...
											className="flex h-10 w-10 items-center justify-center rounded-full bg-[#0077b5]/10 text-[#0077b5] border border-[#0077b5]/20 hover:bg-[#0077b5] hover:text-white transition-all hover:scale-110"
										>
									...
									{formData.github && (
										<a
											...
											target="_blank"
											rel="noopener noreferrer"
											className="flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white border border-white/20 hover:bg-white hover:text-black transition-all hover:scale-110"
										>
									...
									{formData.instagram && (
										<a
											...
											target="_blank"
											rel="noopener noreferrer"
											className="flex h-10 w-10 items-center justify-center rounded-full bg-pink-500/10 text-pink-500 border border-pink-500/20 hover:bg-pink-500 hover:text-white transition-all hover:scale-110"
										>
...
export default function MembersPage() {
	...
	const handleUpdateMember = async (
		...
		try {
			...
			if (
				updatedMember.isDerived &&
				updatedMember.parentId &&
				updatedMember.category === "Member"
			) {
				...
				const currentList = parent.rawGeneralMembers
					? parent.rawGeneralMembers
							.split(/[\n,]+/)
							.map((s) => s.trim())
							.filter((s) => s.length > 0)
					: [];
	...
	const handleDeleteMember = async (id: string, version: number) => {
		try {
			...
			if (
				memberToDelete.isDerived &&
				memberToDelete.parentId &&
				memberToDelete.category === "Member"
			) {
			...
			const leadsWithThisMember = members.filter(
				(m) =>
					!m.isDerived &&
	...
	const handleCreateMember = async (newMember: Member, file?: File | null) => {
		try {
			...
			if (
				...
				newMember.role.toLowerCase().includes("general member")
			) {
				...
				const candidates = members.filter((m) => {
					...
					const isTeamMatch =
						memberTeam.includes(searchTeam) || searchTeam.includes(memberTeam);
				...
				let teamLead = candidates.find((m) => {
					...
					const isCategoryLeader =
						m.category === "Lead" || m.category === "Executive";
					const isExplicitLeader = definedLeadRoles.some(
						(r) => m.role.trim().toLowerCase() === r.toLowerCase(),
					);
				...
				if (!teamLead) {
					alert(
						`No Team Lead found for Team: ${targetTeam}, Year: ${targetYear}. Please create a Leader first.`,
					);
					return;
				}

				// Append new name to existing list
				const currentList = teamLead.rawGeneralMembers
					? teamLead.rawGeneralMembers
							.split(/[\n,]+/)
							.map((s) => s.trim())
							.filter((s) => s.length > 0)
					: [];
	...
	const structuredData = useMemo(() => {
		...
		const years = Array.from(new Set(nonFaculty.map((m) => m.year))).sort(
			(a, b) => b - a,
		);

		const yearGroups = years.map((year) => {
			const yearMembers = nonFaculty.filter((m) => m.year === year);
			const executive = yearMembers
	...
	const renderCard = (member: Member) => {
		...
		const borderColor =
			member.category === "Member" ? "border-transparent" : "border-white/5";
		const cardBg =
			member.category === "CoLead"
				? "bg-[#121214]/80"
		...
		return (
			...
			>
				{!isTiny && (
					<div
						className={`relative mb-3 ${isBig ? "h-20 w-20" : "h-16 w-16"} overflow-hidden rounded-full border-2 border-[#18181b] shadow-lg shadow-black/50 ring-2 ring-white/5 transition-all duration-300 ${styles.ring}`}
					>
						{member.photoUrl ? (
							<img
								src={member.photoUrl}
							...
							<div
								className={`flex h-full w-full items-center justify-center text-gray-500 ${isBig ? "bg-gradient-to-br from-indigo-900/20 to-purple-900/20" : "bg-white/5"}`}
							>
								<span
									className={`${isBig ? "text-xl" : "text-lg"} font-bold select-none`}
								>
				...
				<h3
					className={`font-bold text-white mb-1 line-clamp-1 w-full px-1 ${isTiny ? "text-sm text-left" : "text-base"}`}
					title={member.name}
				>
					{member.name}
				</h3>
				...
				<span
					className={`mb-2 inline-block rounded py-0.5 px-1.5 text-[10px] font-bold uppercase tracking-wider truncate max-w-full ${styles.badge} ${isTiny ? "w-full text-left" : ""}`}
				>
					{member.role}
				</span>

				<div
					className={`absolute inset-0 -z-10 rounded-2xl bg-gradient-to-b ${styles.glow} to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100 pointer-events-none`}
				/>
			</div>
		);
	};
				...
				{isSearching ? (
					...
					<div className="space-y-16">
						...
						{structuredData!.yearGroups.map((group) => (
							<div
								key={group.year}
								className="relative pt-8 border-t border-white/5"
							>
								{/* Year Header */}
								...
								<div className="space-y-12 pl-0 md:pl-4">
									...
									{group.executive.length > 0 && (
										<section>
											<h4 className="fade-in text-xs font-bold uppercase tracking-widest text-purple-400 mb-4 opacity-80">
												Executive Board
											</h4>
											<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
									...
									{group.leads.length > 0 && (
										<section>
											<h4 className="fade-in text-xs font-bold uppercase tracking-widest text-white/60 mb-4 opacity-80">
												Leads
											</h4>
											<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
									...
									{group.coLeads.length > 0 && (
										<section>
											<h4 className="fade-in text-xs font-bold uppercase tracking-widest text-white/50 mb-4 opacity-80">
												Co-Leads
											</h4>
											<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
									...
									{group.generalByTeam.length > 0 && (
										<section>
											<h4 className="fade-in text-xs font-bold uppercase tracking-widest text-gray-500 mb-6 opacity-80">
												General Members
											</h4>
