"use client";
import { useEffect, useRef, ReactNode, FocusEvent } from 'react';
...
export default function PixelCard({ variant = 'default', gap, speed, colors, noFocus, className = '', children }: PixelCardProps) {
  ...
  return (
    <div
      ref={containerRef}
      className={`w-full h-full relative overflow-hidden border border-[#27272a] rounded-[25px] isolate transition-colors duration-200 ease-[cubic-bezier(0.5,1,0.89,1)] select-none ${className}`}
    ...
    >
      <canvas className="absolute inset-0 w-full h-full block z-10" ref={canvasRef} />
