...
export const exportODListExcel = async (
	...
	import("exceljs")
		.then(async (ExcelJSModule) => {
			...
			sortedGroupKeys.forEach((key, groupIdx) => {
				...
				sheet.getCell(`F${currentRow}`).font = {
					bold: true,
					color: { argb: "FF777777" },
				};
				...
				sheet.getCell(`A${currentRow}`).value =
					"Student Form for ATTENDANCE Request";
